...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // Check if voter ID already exists for this MLA
        if ($voterMaster->voterIdExists($_POST['voter_id'], $_POST['mla_id'])) {
        ...
        $result = $voterMaster->create($data);
...
$pageTitle = 'Add New Voter - Election Management System';
include 'header.php';
?>
...
<div class="page-header">
    <h1>➕ Add New Voter</h1>
    <p>Create a new voter record with complete information</p>
</div>
...
<div class="form-container">
    ...
    <form method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="voter_id" name="voter_id" 
                       value="<?php echo htmlspecialchars($_POST['voter_id'] ?? ''); ?>" 
                       required>
                ...
                <select id="mla_id" name="mla_id" required>
                    ...
                    <?php foreach ($mlas as $mla): ?>
                        <option value="<?php echo $mla['mla_id']; ?>" 
                            ...
                            <?php echo htmlspecialchars($mla['mla_constituency_name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="text" id="husband_name" name="husband_name" 
                       value="<?php echo htmlspecialchars($_POST['husband_name'] ?? ''); ?>">
                ...
                <select id="gender" name="gender" required>
                    ...
                    <?php foreach ($genders as $gender): ?>
                        <option value="<?php echo $gender; ?>" 
                                <?php echo ($_POST['gender'] ?? '') === $gender ? 'selected' : ''; ?>>
                            <?php echo $gender; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                ...
                <select id="booth_id" name="booth_id">
                    ...
                    <?php foreach ($booths as $booth): ?>
                        <option value="<?php echo $booth['booth_id']; ?>" 
                                <?php echo ($_POST['booth_id'] ?? '') === $booth['booth_id'] ? 'selected' : ''; ?>>
        ...
        <div class="form-row-3">
            <div class="form-group">
                ...
                <input type="text" id="ward_no" name="ward_no" 
                       value="<?php echo htmlspecialchars($_POST['ward_no'] ?? ''); ?>">
            </div>
            
            <div class="form-group">
                <label for="part_no">Part Number</label>
                <input type="text" id="part_no" name="part_no" 
                       value="<?php echo htmlspecialchars($_POST['part_no'] ?? ''); ?>">
            </div>
        ...
        <div class="form-actions">
            <a href="voter_view.php" class="btn btn-secondary">Cancel</a>
...
<script>
    ...
    document.getElementById('booth_id').addEventListener('change', function() {
        ...
        if (boothId) {
            // You can add AJAX call here to get booth details
